...
## 답안 확신도 안내 개선 (2026-09-25)

- 문제 제출 전 `찍었어요`·`헷갈려요`·`확실해요`를 모바일에서도 한눈에 읽히는 선택 버튼으로 보여준다. 맞혔더라도 찍거나 헷갈린 문제는 복습한다는 이유, 점수에는 영향이 없다는 사실, 선택은 필수가 아니라는 점을 함께 안내한다.
- 제출 후에는 저장된 `Attempt`의 정오 및 확신도에 따라 실제 복습 반영을 쉬운 말로 설명한다. 찍어서 맞힌 경우 다음 날 복습, 헷갈렸지만 맞힌 경우 다음 날 확인, 오답인 경우 오답 복습을 명시한다. 확신도 미선택을 임의로 `헷갈려요`로 추정하지 않는다.
- 기존 채점, `Attempt` 저장, 혼동 카드, 복습 일정, 오답 분석 구조는 유지하며 데이터 migration은 필요 없다.
...
### 물류관리사 집중 과목 선택 (2026-09-25)

- 물류관리사 학습 홈의 첫 진입에서 5개 시험과목 중 공부할 과목을 직접 고른다.
  선택은 `CourseFocus`에 사용자·자격증별로 저장하며 학습 홈에서 언제든 변경한다.
  선택은 풀이·시험 기록을 이동하거나 삭제하지 않는다.
- 선택 과목의 오늘 추천, 학습 진행, 문제 검색, 약점 안내와 해당 과목 목차를 우선
  보여준다. 아직 해당 과목 풀이 기록이 없으면 약점을 추측하지 않고 학습 시작을
  안내한다. 목차의 각 단원에서 이론 또는 해당 단원 문제로 이동할 수 있다.
- 한 과목만 데이터가 있는 개발/테스트 환경에서는 선택 화면을 강제하지 않는다.
  물류관리사 실전 모의고사와 최종 합격 판정은 선택과 무관하게 기존 5과목 전체 기준이다.
  새 자격증의 과목 목록은 Mission의 `course`/`chapter_code`에서 파생할 수 있다.
